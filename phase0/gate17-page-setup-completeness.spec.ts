...
// Gate 17 -- the Page Setup Completeness sub-project's real end-to-end proof.
//
...
async function openInSplitMode(frontmatter: string, label: string): Promise<Fixture> {
  const marker = `Gate17 ${label} ${Date.now()}`
  const filename = `gate17-${label}-${Date.now()}.md`
...
test.describe('Gate 17: Page Setup completeness', () => {
  test('header and footer content really render into the Paged.js margin boxes', async () => {
    ...
    console.log('Gate 17 header/footer probe:', JSON.stringify(probe, null, 2))
  ...
  test('running content does NOT change the page count', async () => {
    ...
    console.log(
      `Gate 17 page counts: running content off=${offProbe.pageCount}, on=${onProbe.pageCount}`
    )
  ...
  test('roman page numbers resolve as roman numerals', async () => {
    ...
    const probe = await pollPreview(marker)
    console.log('Gate 17 roman footer:', probe.bottomCenter)
  ...
  test('a Custom page size renders at its own real pixel dimensions', async () => {
    ...
    console.log('Gate 17 custom sheet:', JSON.stringify(customProbe.sheet))
    ...
    console.log('Gate 17 Letter control sheet:', JSON.stringify(letterProbe.sheet))
  ...
  test('a document theme changes real typography, on both surfaces alike', async () => {
    ...
    console.log(`Gate 17 theme body size: preview=${probe.bodyFontSize}, editor=${editorBody}`)
  ...
  test('a selected font family resolves on both surfaces', async () => {
    ...
    console.log(`Gate 17 font: preview=${probe.bodyFontFamily}, editor=${editorFont}`)
  ...
  test('running content resolves to real page numbers in a real exported PDF', async () => {
    ...
    const content = [
      '---',
      'header: true',
      'headerCenter: Gate 17 Running Header',
      'footer: true',
      'footerCenter: Page {n} of {total}',
      '---',
      '',
    ...
    const targetPath = join(fixtureDir, 'gate17-running-content.pdf')
