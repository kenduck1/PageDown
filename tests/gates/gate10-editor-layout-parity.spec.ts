...
// own single argument rather than referenced from inside the callback. They
// are the right ones for THIS gate specifically because REPORT_TEMPLATE
// carries no page-config frontmatter at all; the per-document (A4) case is
...
// Same pattern as tests/gates/gate9-thumbnail-concurrency.spec.ts's own
...
test('Gate 10: editor/paginator layout parity for the real mounted Milkdown canvas', async () => {
  ...
  try {
    ...
    // tests/spike/gate3-layout-parity.spec.ts's identical, already-working pattern.
