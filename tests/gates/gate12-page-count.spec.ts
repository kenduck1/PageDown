...
// same architectural risk tests/gates/gate9-thumbnail-concurrency.spec.ts already
// proved for `getThumbnail`'s harness.
//
// Uses `launchIsolatedApp` (tests/gates/electron-launch.ts), not a bare
...
// tests/gates/corpus/generate-long.ts.
...
test.describe('Gate 12: real page counts via the dedicated getPageCount harness', () => {
  ...
  test('a genuinely long document paginates to many pages, not a hardcoded/stubbed value', async () => {
    ...
    // repo's own committed tests/gates/results/gate2-timing.json shows the
  ...
  test('concurrent getPageCount calls for differently-sized documents each resolve with their OWN correct count', async () => {
    ...
    // swapped one), the same property tests/gates/gate9-thumbnail-concurrency.spec.ts
