...
// style — which is what `DEFAULT_STYLE` (tests/gates/gate-geometry.ts) hands this
...
// normalized by stripping all whitespace: 9 of 15 files match EXACTLY,
// character-for-character. 5 more do not match exactly, but in EVERY case
// the on-screen text is a subsequence of the exported PDF's text (see
...
//     `out/pagination-render/` bundle, not `tests/gates/corpus/assets/`; a real,
    ...
    for (const file of files) {
      ...
      const evalResult = await app.evaluate(
        async (_electronNS, { html, geometry, documentStyle, gluedTokens }) => {
          ...
          // see tests/gates/gate26-math-equations.spec.ts's own header comment for
      ...
      // (it only serves `out/pagination-render/`, not `tests/gates/corpus/
      ...
      if (file === 'mixed.md' || file === 'images-and-diagrams.md') {
        ...
        for (const img of imgInfo) {
          expect(
            ...
            `${file}: <img src="${img.src}"> is expected to have FAILED to load in this harness (naturalWidth/Height 0 despite complete=true) — this is the real, separate, previously-undocumented gap this task found (the pagedown-render:// scheme doesn't serve tests/gates/corpus/assets/); if this now loads for real, the image-serving gap has been closed and this assertion (and the alt-text-fallback explanation for this file's SUBSEQUENCE_ONLY_FILES membership) need updating`
          ).toBe(true)
        }
      }
      ...
      perFileResults.push({
        ...
        pdfjsPageCount,
        pageCountMatch:
          pdfLibPageCount === onscreenPageCount && pdfjsPageCount === onscreenPageCount,
...
test('Gate 4: split-block fragmentation — a table split across a page boundary becomes two separate structure elements, not one linked table', async () => {
  ...
  try {
    ...
    // tests/gates/gate3-mermaid.spec.ts is untouched by that branch and passes on
    ...
    expect(
      structRootRef,
      'generateTaggedPDF: true must produce a real /StructTreeRoot'
    ).toBeTruthy()
