...
test('Gate 6: render every break-quality fixture and capture per-page structure for manual review', async () => {
  ...
  try {
    ...
    // oversized-diagram comment (tests/gates/gate3-mermaid.spec.ts) and to Task
    ...
    // required V1 fix". Measured directly in tests/gates/gate3-mermaid.spec.ts
...
test('Gate 6: KeepWithNextHandler prevents a stranded heading in mermaid-diagrams.md, the one fixture that spans pages', async () => {
  ...
  try {
    ...
    // structure (tests/gates/corpus/mermaid-diagrams.md: three "# <Title>" H1s,
...
test('Gate 6: TableContinuationHandler repeats the header on a simple 2-page split, with no clipping or row misplacement', async () => {
  ...
  try {
    ...
    // fragmentation test (tests/gates/gate4-export.spec.ts) — 35 rows forces a
    ...
    expect(
      perPage[0].isRepeatedHeader,
      "page 1's header is the ORIGINAL, not a repeated clone"
    ).toBe(false)
