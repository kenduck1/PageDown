...
// gate7-tests/spike/gate7-phase2 message handlers) for the full Paged.js
...
// exactly tests/gates/corpus/generate-long.ts's generated shape — so this can't
...
test('Gate 7: attempt incremental re-layout from a retained breakToken', async () => {
  ...
  try {
    ...
    if (phase1.sectionNumberAtBreakpoint == null) {
      ...
      throw new Error(
        'Phase 1 could not determine a section number near the captured breakToken — see tests/gates/results/gate7-findings.json for partial results'
      )
    }
    ...
    console.log(
      'Gate 7 phase 2 retained-prefix equivalence check:',
      JSON.stringify(prefixComparison)
    )
